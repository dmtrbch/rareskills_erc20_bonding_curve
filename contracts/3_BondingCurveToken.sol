...
import "./2_BancorBondingCurve.sol";
...
contract MyToken is BancorBondingCurve, ERC20, ERC20Burnable, Ownable {
    uint256 constant private SCALE = 10**18;
    uint256 constant private WITHDRAWAL_FEE = 1000;
    uint256 constant private DENOMINATOR = 10_000;
    uint256 constant private RESERVE_RATIO = 500_000;
    uint256 constant private INITIAL_TOKEN_SUPPLY = 100_000;
        ...
        returns (uint256)
    {
        ...
        require(balanceOf(msg.sender) >= _amount, "ERC20: Insufficient tokens to burn.");
    ...
    function _calculateFee(uint256 _amount) 
        ...
        returns (uint256) 
    {
        require((_amount * WITHDRAWAL_FEE) >= DENOMINATOR, "ERC20: Token sell amount must be bigger.");
